...
const Layout: React.FC<LayoutProps> = ({
  ...
  return (
    <div className={cn("min-h-screen flex flex-col lg:flex-row w-full flex-1 overflow-hidden", darkMode ? "dark bg-slate-900" : "bg-slate-50")}>
        ...
        <SidebarBody className="justify-between gap-10">
          <div className="flex flex-col flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar">
            ...
            <div className="flex items-center gap-3 px-2 mb-8">
              ...
              />
              <span className={cn("text-2xl font-bold tracking-tight text-slate-800 dark:text-white transition-opacity duration-300", open ? "opacity-100" : "opacity-0 hidden lg:block")}>
                Trocô
              </span>
            </div>
            ...
            <SidebarLink
              link={{
                ...
                icon: (
                  <img
                    src={user.avatarUrl}
                    className="h-8 w-8 flex-shrink-0 rounded-full object-cover border-2 border-white dark:border-slate-800"
                    alt="Avatar"
                  />
                ),
              }}
      ...
      <main className="flex-1 flex flex-col min-h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-300 relative overflow-y-auto w-full">
        {/* Header Content */}
        <header className="sticky top-0 z-30 h-16 lg:h-20 px-4 lg:px-10 flex items-center justify-between
          ...
          <div className="flex items-center gap-3">
            <h1 className="text-lg lg:text-2xl font-bold text-slate-800 dark:text-white truncate lg:ml-0 ml-12">
              {getPageTitle()}
            </h1>
          </div>
