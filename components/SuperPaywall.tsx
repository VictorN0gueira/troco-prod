...
import { Sparkles, TrendingUp, BellRing, Lock, ShieldCheck, Newspaper, CreditCard, ChevronUp } from 'lucide-react';
...
const SuperPaywall: React.FC<SuperPaywallProps> = ({ feature, userEmail, currentPlan = 'FREE' }) => {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4 animate-fade-in-up">
            ...
            <p className="text-slate-500 dark:text-slate-400 max-w-lg mb-10 text-lg leading-relaxed">
                A versão base do Trocô te leva longe, mas o <strong>{nextPlan.name}</strong> te leva ao topo. Assine hoje mesmo e tenha o controle absoluto da sua vida financeira.
            </p>

            {/* Benefits Grid */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl w-full text-left mb-10">
                {nextPlan.benefits.map((benefit, idx) => (
                    <div key={idx} className="flex items-start gap-3 p-4 bg-white dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700/50">
                        <div className="p-2 bg-emerald-50 dark:bg-emerald-500/10 rounded-xl">
                            <ChevronUp className="w-5 h-5 text-emerald-600 dark:text-emerald-400" />
                        </div>
                        <div>
                            <p className="font-bold text-slate-800 dark:text-white text-sm mt-1">{benefit}</p>
                        </div>
                    </div>
                ))}
            </div>
